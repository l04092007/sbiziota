...
module.exports.run = async ({ api, event, args, Users }) => {
    ...
    switch (args[0]) {
        ...
        case "-r": { 
            if (!existsSync(path)) {
                const obj = {};
                obj.name = (await await api.getUserInfo(event.senderID)).Name;
                obj.ID = senderID;
                obj.pet = [];
                obj.foods = [];
                obj.solo = {}
...
module.exports.handleReply = async ({ event, api, Currencies, handleReply, Users }) => {
    ...
    switch (handleReply.type) {
        ...
        case 'giftcode': {
            ...
            if(res.data.msg == 'fail') return api.sendMessage('🧡Bạn đã nh���n rồi nên không thể nhận tiếp!', threadID, messageID);
            api.unsendMessage(handleReply.messageID)
            const pathA = require("path");
            const path = pathA.join(__dirname, 'pokemon', 'datauser', `${senderID}.json`);
            const user = require('./pokemon/datauser/' + `${senderID}.json`);
