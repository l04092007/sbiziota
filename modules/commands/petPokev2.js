...
module.exports.handleEvent = async function({ api, event, Currencies }) {
    ...
    if(body.toLowerCase() == 'my pokemon') {
        ...
        for (let i of user.pet) {
            msg += `[${++ii}]. ${i.name} - ${i.coins}$\n🐳Type: ${i.type}\n🧡HP: ${fixed(i.HP)}\n🗡Attack: ${fixed(i.Attack)}\n🛡Defense: ${fixed(i.Defense)}\n⚡️Speed: ${fixed(i.Speed)}\n📌Skill: ${i.skill.join(', ')}\n\n`
        }
...
module.exports.run = async ({ api, event, args, Users }) => {
    ...
    switch (args[0]) {
        ...
        case 'rank': {
            ...
            var msg = '========[TOP RANK POKEMON]========\n'
            for( let i = 0; i < dem.length; i++) {
                if(i < 15) {
                    msg += `Name: ${dem[i].name}\nSố pokemon: ${dem[i].pet.length}\nWin: ${dem[i].solo.win}\nLose: ${dem[i].solo.lose}\nDraw: ${dem[i].solo.draw}\n\n`
                }
            }
        ...
        case '-l': {
            ...
            for (let i of listPoke) {
                index++
                var msg = `🔍ID: ${index}\n🕵️‍♀️NamePoke: ${i.name} - ${fixed(i.coins)}$\n🧡HP: ${fixed(i.power.HP)}\n🗡Attack: ${fixed(i.power.Attack)}\n🛡Defense: ${fixed(i.power.Defense)}\n⚡️Speed: ${fixed(i.power.Speed)}\n\n`;
                list.push(msg)
            }
            var page = 1;
...
module.exports.handleReply = async ({ event, api, Currencies, handleReply, Users }) => {
    ...
    switch (handleReply.type) {
        case 'infoPet': {
            ...
            var msg = {body: `${choosePet.name} - ${choosePet.coins}$\n🐳Type: ${choosePet.type}\n🧡HP: ${choosePet.HP}\n🗡Attack: ${choosePet.Attack}\n🛡Defense: ${choosePet.Defense}\n⚡️Speed: ${choosePet.Speed}\n📌Skill: ${choosePet.skill.join(', ')}\n\n`, attachment: image}
            return api.sendMessage(msg, threadID, messageID);
        }
        case 'spadmin': {
            switch (body) {
        ...
        case 'petFoods': {
            ...
            if(pet[parseInt(choose[0])-1].type.indexOf(foods[parseInt(choose[1])-1].type) !== -1) {
                var HP = ((foods[parseInt(choose[1])-1].HP) * 1.3).toFixed(0)
                var Attack = ((foods[parseInt(choose[1])-1].Attack) * 1.3).toFixed(0)
                var Defense = ((foods[parseInt(choose[1])-1].Defense) * 1.3).toFixed(0)
                var Speed = ((foods[parseInt(choose[1])-1].Speed) * 1.3).toFixed(0)
                var coins = 5000
            } else {
        ...
        case 'choose_buff': {
            ...
            var poke = user.pet[index]
                poke.HP = (poke.HP + poke.HP * 40/100).toFixed(0)
                poke.Attack = (poke.Attack + poke.Attack * 40/100).toFixed(0)
                poke.Defense = (poke.Defense + poke.Defense * 40/100).toFixed(0)
                poke.Speed = (poke.Speed + poke.Speed * 40/100).toFixed(0)
                poke.coins = (poke.coins + poke.coins * 20/100).toFixed(0)
            writeFileSync(path, JSON.stringify(user, null, 2));
            api.unsendMessage(handleReply.messageID)
                            ...
                            setTimeout( async function () {
                                ...
                                setTimeout( function () {
                                    ...
                                    setTimeout( async function () {
                                        ...
                                        const pathA = require("path");
                                        global.pokemon.delete(threadID);
                                        if(parseInt(TB_1) > parseInt(TB_2)) {
                                            ...
                                            var find = user_1.pet.find(i => i.name == index_1.choose.msg.name)
                                                find.HP = parseInt(find.HP) + parseInt(fixed(find.HP * 5/100));
                                                find.Attack = parseInt(find.Attack) + parseInt(fixed(find.Attack * 5/100));
                                                find.Defense = parseInt(find.Defense) + parseInt(fixed(find.Defense * 5/100));
                                                find.Speed = parseInt(find.Speed) + parseInt(fixed(find.Speed * 5/100));
                                                find.coins = parseInt(find.coins) + parseInt(fixed(find.coins * 5/100));
                                            var win = user_1.solo
                                                win.win = win.win + 1
                                            var find = user_2.pet.find(i => i.name == index_2.choose.msg.name)
                                                find.HP = fixed(find.HP - find.HP * 5/100);
                                                find.Attack = fixed(find.Attack - find.Attack * 5/100)
                                                find.Defense = fixed(find.Defense - find.Defense * 5/100);
                                                find.Speed = fixed(find.Speed - find.Speed * 5/100);
                                                find.coins = fixed(find.coins - find.coins * 5/100);
                                        ...
                                        else if(parseInt(TB_1) < parseInt(TB_2)) { 
    ...
    switch (handleReaction.type) {
        ...
        case 'openBox': {
            ...
            user.pet.push({
                name: rd.name,
                type: rd.type,
                HP: (rd.power.HP * handleReaction.tpy).toFixed(0),
                Attack: (rd.power.Attack * handleReaction.tpy).toFixed(0),
                Defense: (rd.power.Defense * handleReaction.tpy).toFixed(0),
                Speed: (rd.power.Speed * handleReaction.tpy).toFixed(0),
                skill: skill,
            ...
            writeFileSync( __dirname + "/pokemon/cache/pokemon.png", Buffer.from(pokemon, "utf-8") );
            image.push(createReadStream(__dirname + "/pokemon/cache/pokemon.png"));
            var msg = {body: `🎉Xin chúc mừng!\n☑️Bạn mở trúng ${rd.name}.\nType: ${rd.type}\n🔍Chỉ số: \n🧡HP: ${(rd.power.HP* handleReaction.tpy).toFixed(0)}\n🗡Attack: ${(rd.power.Attack* handleReaction.tpy).toFixed(0)}\n🛡Defense: ${(rd.power.Defense* handleReaction.tpy).toFixed(0)}\n⚡️Speed: ${(rd.power.Speed* handleReaction.tpy).toFixed(0)}\n📌Skill: ${skillS.replace(/,\s*$/, "")}\n💰Coins: ${handleReaction.money}$\n☑️Đã thêm pokemon vào kho đồ của bạn!`, attachment: image}
            return api.sendMessage(msg, threadID, messageID);
        }
    }
}