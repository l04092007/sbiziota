...
module.exports.handleReply = async function ({
    ...
    try {
        const result = await axios.get(`https://api-rosie.j-jrt-official.repl.co/sing?link=https://www.youtube.com/watch?v=${handleReply.link[event.body - 1]}`);
...
module.exports.run = async function ({
  ...
  if (global.client.voice == true) return api.sendMessage("Hệ thống đang xử lý yêu cầu từ box khác, vui lòng quay lại sau", event.threadID, event.messageID);
        global.client.voice = true;
    const {
        createReadStream,
        createWriteStream,
        unlinkSync,
    ...
    if (args.join(" ").indexOf("https://") == 0) {
        ...
        const result = await axios.get(`https://api-rosie.j-jrt-official.repl.co/sing?link=https://www.youtube.com/watch?v=${linkUrlSing}`);
        path1 = __dirname + `/cache/${event.senderID}.m4a`
        const getms = (await axios.get(`${result.data.link}`, {
            responseType: "arraybuffer"
        })).data;
